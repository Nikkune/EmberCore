...
export class BadgeComponent extends BaseComponent<BadgeProps, UIDrawSurface> {
	...
	public render(context: RenderContext<UIDrawSurface>) {
		...
		const contentRect = drawBox(
			context.draw,
			createOptions<DrawBoxOptions>({
				rect: this.rect,
				style: boxStyle,
			})
	...
	private resolveTextStyle(style: BadgeStyle | undefined, contextTheme: UIContext["theme"]): TextStyle {
		...
		return {
			...style?.text,
			foregroundColor:
				style?.text?.foregroundColor ??
				style?.foregroundColor ??
				toneColors.foregroundColor,
			backgroundColor:
				style?.text?.backgroundColor ??
	...
	private resolveBoxStyle(style: BadgeStyle | undefined, theme: UIContext["theme"]): BoxStyle {
		...
		return {
			...style,
			padding: style?.padding ?? {horizontal: 1},
	...
	private resolveBorderCharacters(style: BadgeStyle | undefined, theme: UIContext["theme"],): BorderCharacters | undefined {
		...
		return getBorderCharacters(
			theme,
			style.border.preset ?? "ascii",
		);
	}

	private resolveToneColors(style: BadgeStyle | undefined, theme: UIContext["theme"]): { foregroundColor: Color; backgroundColor: Color; } {
		switch (style?.tone) {
			case "primary":
				return {
