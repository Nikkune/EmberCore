...
export function drawBorder(draw: UIDrawSurface, options: DrawBorderOptions): void {
	...
	const top =
		rect.width === 1 ? border.vertical : border.topLeft + repeat(border.horizontal, Math.max(0, rect.width - 2)) + border.topRight;

	const bottom =
		rect.width === 1
			? border.vertical
			: border.bottomLeft + repeat(border.horizontal, Math.max(0, rect.width - 2)) + border.bottomRight;
