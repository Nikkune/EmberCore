...
export function wrapText(
	...
	for (let sourceLineIndex = 0; sourceLineIndex < sourceLines.length; sourceLineIndex += 1) {
		...
		const wrappedLines =
			wrap === "word"
				? wrapLineByWord(sourceLine, width)
				: wrapLineByCharacter(sourceLine, width);
...
export function resolveTextLines(
	options: ResolveTextLinesOptions,
): WrappedLine[] {
...
export function drawText(
	context: RenderContext<UIDrawSurface>,
	...
	const targetRect = clipRect
		? intersectRects(options.rect, clipRect)
		: options.rect;
...
export function fillRect(
	...
	const targetRect = options.clipRect
		? intersectRects(options.rect, options.clipRect)
		: options.rect;
...
export function drawBorder(
	...
	const {rect, border, foregroundColor, backgroundColor, clipRect} = options;
