import type {BaseProps, ComponentKind, InvalidationRequest, LayoutConstraints, MeasuredSize, Point, Rect, RenderContext, UIComponent, UIContext, UIEvent, UIEventBus, UIEventHandler, UIEventMap, UIInvalidator} from '@modules/ui/types';
...
export abstract class BaseComponent<TProps extends BaseProps = BaseProps, TDraw = unknown> implements UIComponent<TDraw> {
	...
	public hitTest(point: Point): boolean {
		return (this.visible && point.x >= this.rect.x && point.y >= this.rect.y && point.x < this.rect.x + this.rect.width && point.y < this.rect.y + this.rect.height);
	}
